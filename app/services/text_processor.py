...
from app.services.ai_service import call_google_ai_async
...
SYSTEM_INSTRUCTION = "Você é um assistente de estudos de IA para uma plataforma educacional. Sua função é ajudar os alunos a entenderem seus materiais de estudo. Você deve sempre responder em português do Brasil (pt-BR), ser claro, didático e estruturado."
...
async def _summarize_chunk(chunk: str) -> str:
    ...
    return await call_google_ai_async(prompt, SYSTEM_INSTRUCTION)
...
async def summarize_text_map_reduce(chunks: List[str], level: str) -> str:
    ...
    final_summary = await call_google_ai_async(reduce_prompt, SYSTEM_INSTRUCTION)
...
def _create_document_id(chunks: List[str]) -> str:
    """Cria um ID de documento consistente gerando um hash SHA-256 do conteúdo."""
...
async def generate_summary_with_cache(db: AsyncSession, chunks: List[str], level: str) -> str:
    """
    Orquestra a geração de resumo, utilizando o cache e com tratamento de timeouts.
    """
    ...
    try:
        ...
        # Captura o erro de timeout e o converte em uma resposta HTTP amigável
        raise HTTPException(
            status_code=status.HTTP_504_GATEWAY_TIMEOUT,
            detail="A geração de conteúdo demorou demais para responder. Tente novamente com um documento menor."
        )
