...
const App: React.FC = () => {
  ...
  const handleFileChange = useCallback(async (file: File | null) => {
    ...
    try {
      // Usar caminho relativo para o backend. Funciona perfeitamente no Render.
      const backendUrl = '/api/v1/upload-pdf/';
  ...
  const handleGenerate = useCallback(async (featureToGenerate: Feature) => {
    if (!inputText.trim() && featureToGenerate !== 'resumo') {
      setError('Por favor, carregue e processe um PDF antes de gerar conteúdo.');
      return;
    }
     if (featureToGenerate === 'resumo' && textChunks.length === 0) {
      setError('Por favor, carregue e processe um PDF antes de gerar um resumo.');
      return;
    }
    ...
    try {
      let result;
      // O texto completo é usado para features mais simples, os chunks para o resumo complexo
      const contextText = textChunks.join('\n\n');
      ...
      switch (featureToGenerate) {
        case 'resumo':
          ...
          break;
        case 'quiz':
          result = await generateQuiz(contextText, studentLevel);
          setGeneratedContent(prev => ({ ...prev, quiz: result as QuizQuestion[] }));
          break;
        case 'flashcards':
          result = await generateFlashcards(contextText, studentLevel);
          setGeneratedContent(prev => ({ ...prev, flashcards: result as Flashcard[] }));
          break;
        case 'plano':
          result = await generateStudyPlan(contextText, studentLevel, studyPlanDays);
          setGeneratedContent(prev => ({ ...prev, plano: result as string }));
          break;
        case 'perguntas':
          result = await generateQuestions(contextText, questionCount, difficultyLevel);
            ...
            <div className="sticky top-8 space-y-6">
              ...
              <div className="bg-surface p-4 rounded-xl shadow-subtle">
                ...
                <FeatureTabs 
                  ...
                  isInputEmpty={textChunks.length === 0}
                />
              </div>
            </div>
          </div>
